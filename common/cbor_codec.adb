...
package body CBOR_Codec is
   ...
   subtype Additional_Value_Type is Unsigned_32 range 0 .. 31;
   ...
   procedure Encode_Additional_Data (Value : Unsigned_32; Major : Major_Type) is
      Encoding_Unsigned_8 : Encoding_Type;
   begin
      Encoding_Unsigned_8.Major := Major;
      if Value <= Additional_8Bit - 1 then
         Encoding_Unsigned_8.Value := Value;
         Write (Encoding_Unsigned_8.Val);
      elsif Value <= Unsigned_32 (Unsigned_8'Last) then
         Encoding_Unsigned_8.Value := Additional_8Bit;
         Write (Encoding_Unsigned_8.Val);
         Write (Unsigned_8 (Value));
      elsif Value <= Unsigned_32 (Unsigned_16'Last) then
         Encoding_Unsigned_8.Value := Additional_16Bit;
         Write (Encoding_Unsigned_8.Val);
         Write (Unsigned_8 (Value / 2**8));
         Write (Unsigned_8 (Value mod 2**8));
      elsif Value <= Unsigned_32'Last then
         Encoding_Unsigned_8.Value := Additional_32Bit;
         Write (Encoding_Unsigned_8.Val);
   ...
   procedure Encode_Integer (Value : Integer) is
      Major : Major_Type;
      Positive_Value : Unsigned_32;
   begin
      if Value >= 0 then
         Major := Unsigned_Integer;
         Positive_Value := Unsigned_32 (Value);
      else
         Major := Negative_Integer;
         Positive_Value := Unsigned_32 (-1 - Value);
      end if;
   ...
   begin
      Encode_Additional_Data (Unsigned_32 (Count), Item_Array);
   end Encode_Array;

   procedure Encode_Map (Count : Natural) is
   begin
      Encode_Additional_Data (Unsigned_32 (Count), Item_Map);
   end Encode_Map;

   procedure Encode_Tag (Value : Natural) is
   begin
      Encode_Additional_Data (Unsigned_32 (Value), Tag);
   end Encode_Tag;
   ...
   function Decode_Additional_Data (Value : out Unsigned_32;
      Encoding_Byte : Encoding_Type) return Boolean is
      Success : Boolean := False;
   begin
      case Encoding_Byte.Value is
         when  Additional_8Bit =>
            Value := Unsigned_32 (Read);
            return True;
         when Additional_16Bit =>
            Value := Unsigned_32 (Read) * 2**8 +
               Unsigned_32 (Read);
            return True;
         when Additional_32Bit =>
            Value := Unsigned_32 (Read) * 2**24 +
               Unsigned_32 (Read) * 2**16 +
               Unsigned_32 (Read) * 2**8 +
               Unsigned_32 (Read);
            return True;
   ...
   begin
      ...
      if Decode_Additional_Data (V, Encoding_Byte) then
   ...
   begin
      ...
      if Encoding_Byte.Major = Byte_String and then
         Decode_Additional_Data (Unsigned_32 (Size), Encoding_Byte) then
         return True;
      else
         return False;
      end if;
   ...
   begin
      ...
      if Encoding_Byte.Major = Item_Array and then
         Decode_Additional_Data (Unsigned_32 (Count), Encoding_Byte) then
         return True;
      else
         return False;
      end if;
   ...
   begin
      ...
      if Encoding_Byte.Major = Tag and then
         Decode_Additional_Data (Unsigned_32 (Value), Encoding_Byte) then
         return True;
      else
         return False;
      end if;
